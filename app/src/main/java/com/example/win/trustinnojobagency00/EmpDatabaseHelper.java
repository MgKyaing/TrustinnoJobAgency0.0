...
public class EmpDatabaseHelper extends SQLiteOpenHelper {
    ...
    public static final String tb_name = "Employers_table";
    public static final String col1 = "ID";
    public static final String col2 = "COMPANY_NAME";
    ...
    public void onCreate(SQLiteDatabase db) {
        db.execSQL("create table " + tb_name + " (ID INTEGER PRIMARY KEY AUTOINCREMENT, COMPANY_NAME TEXT,TITLE TEXT,DESCRIPTION TEXT, DATE TEXT)");
    }
    ...
    public boolean insertData(String com_name, String title, String description, String date) {
        SQLiteDatabase db = getWritableDatabase();
        ContentValues cv = new ContentValues();
        cv.put(col2, com_name);
        cv.put(col3, title);
    ...
    public Cursor getDataByCompanyName(String com_name) {
        SQLiteDatabase db = getWritableDatabase();
        Cursor res = db.rawQuery("SELECT * FROM " + tb_name + " WHERE COMPANY_NAME=" + com_name, null);
        return res;
    }

    public Cursor getDataByTitle(String title) {
        SQLiteDatabase db = getWritableDatabase();
        Cursor res = db.rawQuery("SELECT * FR0OM " + tb_name + " WHERE TITLE=" + title, null);
        return res;
    }

    public boolean updateData(String id, String com_name, String title, String description, String date) {
        SQLiteDatabase db = getWritableDatabase();
        ContentValues cv = new ContentValues();
        cv.put(col1, id);
        cv.put(col2, com_name);
